...
class TestLocaleScript(unittest.TestCase):
    ...
    def test_version_info_with_file_exist(self):
        ...
        with patch('os.path.isfile', return_value=True):
            with patch('sys.argv', test_args):
                locale_1.main()
                self.assertIn('Nicolas Huber, 25061944, Friday 17 May 2024', sys.stdout.getvalue())
    ...
    def test_file_not_found(self):
        ...
        with patch('sys.argv', test_args):
            with self.assertRaises(SystemExit) as cm:
                locale_1.main()
            self.assertIn('Error: The file ./data/non_existent_file.csv does not exist or is not readable.', sys.stdout.getvalue())
